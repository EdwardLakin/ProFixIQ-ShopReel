...
import { cancelAgentTask } from "@/features/shopreel/agents/approval";
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await cancelAgentTask({ shopId, taskId: id, userId, reason: body.reason ?? null, metadata: body.metadata ?? {} });
    ...
    return toEndpointErrorResponse(error, "Failed to cancel agent task");
  }
}
