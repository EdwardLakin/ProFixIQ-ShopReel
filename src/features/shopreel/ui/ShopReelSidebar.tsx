...
export default function ShopReelSidebar() {
  ...
  return (
    <>
      <div
        className="fixed left-0 top-0 z-40 h-full w-8 touch-pan-y sm:w-10"
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-4 left-2 z-[75] flex h-12 min-w-[74px] items-center justify-center gap-2 rounded-full border border-white/15 bg-[#090f21]/90 px-3 text-xs font-medium tracking-[0.08em] text-cyan-50/90 shadow-[0_8px_30px_rgba(0,0,0,.42),0_0_24px_rgba(56,189,248,.2)] backdrop-blur-xl sm:bottom-auto sm:top-6"
      >
        <span className="text-sm leading-none">☰</span><span>Launch</span>
      </button>

      {open ? <button type="button" aria-label="Close navigation menu" className="fixed inset-0 z-[70] bg-[#02040b]/65 backdrop-blur-[1px]" onClick={() => setOpen(false)} /> : null}

      <aside className={`fixed inset-y-0 left-0 z-[80] w-[min(24rem,94vw)] transform overflow-x-hidden border-r border-cyan-100/10 bg-[#040812]/96 p-5 shadow-[30px_0_90px_rgba(0,0,0,.66)] backdrop-blur-2xl transition duration-300 ${open ? "translate-x-0" : "-translate-x-full"}`}>
