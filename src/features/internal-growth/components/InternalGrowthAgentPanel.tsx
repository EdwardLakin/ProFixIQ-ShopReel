...
type Overview = { runs: Array<Record<string, unknown>>; features: Array<Record<string, unknown>>; campaigns: Array<Record<string, unknown>>; drafts: Array<Record<string, unknown>>; signals: Array<Record<string, unknown>>; sources: Array<Record<string, unknown>>; assetPlans?: Array<Record<string, unknown>>; screenshotRequests?: Array<Record<string, unknown>>; renderCompositions?: Array<Record<string, unknown>>; renderJobs?: Array<Record<string, unknown>>; summary?: Record<string, unknown> };
...
export default function InternalGrowthAgentPanel() {
  ...
  return <div className="space-y-4">
    ...
    <div className="rounded border border-white/10 p-3 text-sm"><h3 className="font-semibold">Asset Plans</h3><ul className="space-y-1 text-xs">{(data.assetPlans ?? []).slice(0,8).map((p)=><li key={String(p.id)}>{String(p.title)} · {String(p.status)}</li>)}</ul></div><div className="rounded border border-white/10 p-3 text-sm"><h3 className="font-semibold">Screenshot Requests</h3><ul className="space-y-1 text-xs">{(data.screenshotRequests ?? []).slice(0,8).map((r)=><li key={String(r.id)}>{String(r.title)} · {String(r.viewport)} · {String(r.status)}</li>)}</ul></div><div className="rounded border border-white/10 p-3 text-sm"><h3 className="font-semibold">Render Queue</h3><ul className="space-y-1 text-xs">{(data.renderJobs ?? []).slice(0,8).map((j)=><li key={String(j.id)}>{String(j.status)} · {String(j.progress)}%</li>)}</ul></div></section>
