...
export default async function ShopReelGenerationsPage() {
  ...
  return <GlassShell title="Projects" hidePageIntro><div className="space-y-4"><ShopReelPageHero title="Generations" subtitle="See what needs attention, what is ready, and the next move." actions={[{ label: "Create content", href: "/shopreel/create", primary: true }, { label: "Open render queue", href: "/shopreel/render-queue" }]} /><div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_290px]"><div className="space-y-4"><ShopReelSurface title="Queue health"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{[{label:"Active renders",value:buckets["in-progress"].length},{label:"Queued jobs",value:generations.filter((g)=>normalizeStatus(g.status)==="queued").length},{label:"Needs review",value:buckets.failed.length},{label:"Ready today",value:completedToday}].map((m)=><div key={m.label} className="rounded-xl border border-white/10 bg-black/25 p-3"><p className="text-xs text-white/60">{m.label}</p><p className="mt-1 text-2xl font-semibold text-white">{m.value}</p></div>)}</div></ShopReelSurface>
<ShopReelSectionHeader eyebrow="Generation lanes" title="Active path by stage" subtitle="Keep drafts moving from queue to publish." />
<ShopReelOperationalLanes>
{([ ["In progress", buckets["in-progress"]],["Ready to publish", buckets.completed],["Needs review", buckets.failed],["Drafts", buckets.drafts] ] as const).map(([label,items])=> <div key={label} className="min-w-[min(100%,26rem)] flex-1"><ShopReelSurface title={label} description={`${items.length} item${items.length===1?"":"s"}`} prominence={label==="Needs review"&&items.length?"elevated":label==="Drafts"&&!items.length?"recessed":"normal"} density={items.length>5?"compact":"balanced"}>{items.length===0?<ShopReelEmptyState title={`No ${label.toLowerCase()} right now`} description="Nothing waiting. Move the next draft forward."/>:<div className="grid gap-2">{items.map((g)=><GenerationCard key={g.id} generation={g} />)}</div>}</ShopReelSurface></div>)}
</ShopReelOperationalLanes>
</div><ShopReelActionRail title="Operations rail" items={["Resolve failed jobs before opening new queue load","Review 'Next' actions for each lifecycle stage","Move ready outputs into exports for package checks","Use editor links only for current working generation"]} /></div></div></GlassShell>;
}
