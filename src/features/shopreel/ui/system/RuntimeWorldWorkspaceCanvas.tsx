...
export default function RuntimeWorldWorkspaceCanvas({ entry, children, operatorPanel, panelReveal, continuityRailFocus, immersion, interaction }: { entry: RuntimeWorldEntry; children: ReactNode; operatorPanel: ReactNode; panelReveal: number; continuityRailFocus: number; immersion: RuntimeImmersionState; interaction: RuntimeInteractionState }) {
  ...
  return <section className={`grid gap-4 ${pressureRail}`} style={{ opacity: Math.max(0.82, panelReveal), filter: `contrast(${immersion.atmosphere.panelContrast})`, background: `linear-gradient(180deg, rgba(8,12,20,${0.2 + interaction.continuityAttention * 0.2}), rgba(8,12,20,0))` }}>
    <div className="grid min-h-0 gap-4">
      <div style={{ opacity: immersion.reveal.primaryReveal, transform: immersion.reducedMotion ? "none" : `scale(${0.996 + immersion.reveal.primaryReveal * 0.004})`, filter: `blur(${immersion.reducedMotion ? 0 : (1 - immersion.reveal.primaryReveal) * 2}px)`, boxShadow: `inset 0 0 ${relationshipGlow}px rgba(34,211,238,${0.08 + interaction.guidanceCue.continuityGlow * 0.16})` }}><RuntimeRoutePanelAdapter adapter={{ panelId: primary?.id ?? "primary", route: primary?.route ?? entry.href, title: primary?.title ?? "Workspace", embedMode: "embedded" }}>{children}</RuntimeRoutePanelAdapter></div>
      <div className={`grid gap-4 md:grid-cols-2 ${secondaryClass}`}>{secondary.map((panel, index) => <div key={panel.id} className={choreography.panelPriority.collapsedPanelIds.includes(panel.id) ? "opacity-85" : ""} style={{ opacity: immersion.reveal.secondaryReveal[index] ?? immersion.reveal.secondaryReveal.at(-1) ?? 1, filter: `blur(${immersion.reducedMotion ? 0 : (1 - (immersion.reveal.secondaryReveal[index] ?? 1)) * 1.25}px)`, boxShadow: `inset 0 0 ${relationshipGlow * 0.7}px rgba(125,211,252,${0.05 + interaction.panelRelationshipIntensity * 0.14})` }}><RuntimeRoutePanelAdapter adapter={{ panelId: panel.id, route: panel.route, title: panel.title, embedMode: "embedded" }} /></div>)}</div>
