...
export async function GET(_: Request, { params }: { params: { id: string } }) { await assertInternalGrowthAccess(); const pkg = await getCampaignPackage(params.id); return NextResponse.json({ campaignBrief: pkg.campaign, drafts: pkg.drafts, captions: pkg.drafts, storyboard: pkg.assetPlans, screenshotRequests: pkg.screenshotRequests, renderCompositions: pkg.renderCompositions, renderReadiness: pkg.renderReadiness, brandKitReferences: pkg.brandKit, futurePublishPayload: pkg.futurePublishPayload }); }
