...
export default function CampaignDetailClient({ campaign, items, progress, adaptiveMemory }: { campaign: CampaignRow; items: CampaignItemRow[]; progress: { totalItems: number; completedItems: number; progressPercent: number; totalScenes: number; queuedScenes: number; processingScenes: number; completedScenes: number; failedScenes: number }; adaptiveMemory: AdaptiveMemory }) {
  ...
  return (
    <section className="space-y-6 rounded-[2rem] border border-cyan-200/20 bg-slate-950/95 p-4 shadow-[0_20px_70px_rgba(2,132,199,0.25)] sm:p-6">
      <div className="space-y-5">
        ...
        <div className="rounded-3xl border border-cyan-300/35 bg-cyan-500/10 p-4 sm:p-5">
          ...
          <div className="flex flex-wrap gap-3 [&>*]:min-h-11 [&>*]:w-full sm:[&>*]:w-auto">
        ...
        <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-3 rounded-3xl border border-white/10 bg-slate-950/55 p-4">
            ...
            {pendingTasks.length === 0 ? <div className="text-sm text-white/65">Approval lane is clear.</div> : pendingTasks.slice(0, 4).map((task) => <article key={task.id} className="min-w-0 rounded-2xl bg-black/35 p-3 ring-1 ring-white/10"><div className="flex flex-wrap items-center gap-2"><span className="font-medium text-white break-words">{task.title}</span><GlassBadge tone="muted">Confidence {task.confidence ?? "n/a"}</GlassBadge></div>{task.details ? <p className="mt-1 text-sm text-white/70 break-words">{task.details}</p> : null}<textarea value={taskReason[task.id] ?? ""} onChange={(e) => setTaskReason((prev) => ({ ...prev, [task.id]: e.target.value }))} className="mt-2 w-full rounded-xl border border-white/10 bg-black/35 p-2 text-sm" placeholder="Refinement note" /><div className="mt-2 flex flex-wrap gap-2 [&>*]:min-h-11 [&>*]:w-full sm:[&>*]:w-auto"><GlassButton variant="secondary" onClick={() => void transitionTask(task.id, "approve")}>Approve</GlassButton><GlassButton variant="ghost" onClick={() => void transitionTask(task.id, "reject")}>Refine</GlassButton></div></article>)}
          </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-4">
          ...
          <div className="space-y-2">
            {items.slice(0,5).map((item) => {
              ...
              return <div key={item.id} className="min-w-0 rounded-xl border border-white/10 bg-slate-950 p-3"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="font-semibold text-white break-words">{item.title}</div><div className="text-sm text-white/60 break-words">{intel.hook ?? item.angle}</div><div className="text-xs text-white/60 break-words">Why it fits: {intel.why_it_fits_brief ?? "Mode-aligned angle"}</div><div className="text-xs text-white/60 break-words">Suggested outputs: {Array.isArray(intel.suggested_outputs) ? intel.suggested_outputs.join(", ") : "n/a"}</div><div className="text-xs text-white/60 break-words">Primary CTA: {typeof intel.primary_cta === "string" ? intel.primary_cta : "Get started"}</div></div><div className="flex flex-wrap items-center gap-2 [&>*]:min-h-11 [&>*]:w-full sm:[&>*]:w-auto">{item.status !== "approved" ? <GlassButton variant="secondary" onClick={() => void buildPackage(item.id, "build")}>Approve an angle</GlassButton> : null}{item.status === "approved" && !pkg ? <GlassButton variant="secondary" onClick={() => void buildPackage(item.id, "build")}>Build production package</GlassButton> : null}{pkg && pkgStatus !== "approved" ? <GlassButton variant="secondary" onClick={() => void buildPackage(item.id, "approve")}>Approve package</GlassButton> : null}</div></div>{pkg ? <div className="mt-3 rounded-lg border border-cyan-200/20 p-3 text-xs text-cyan-50"><div className="mb-2 font-semibold">Production package ({pkg.mode ?? campaignMode})</div><div className="space-y-2">{Object.entries(pkg.sections ?? {}).map(([section, value]) => <div key={section} className="rounded-md bg-slate-900/75 p-2"><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-cyan-100/80">{section}</p><p className="mt-1 whitespace-pre-wrap break-words text-cyan-50/90">{normalizeCopyValue(value)}</p></div>)}</div><div className="mt-2 flex flex-wrap gap-2 [&>*]:min-h-11 [&>*]:w-full sm:[&>*]:w-auto"><GlassButton variant="ghost" onClick={() => void copyWithFeedback(`${item.id}-full`, fullPackageText)}>Copy full package</GlassButton><GlassButton variant="ghost" onClick={() => void copyWithFeedback(`${item.id}-caption`, String((pkg.sections?.caption ?? pkg.sections?.facebook_post ?? "")))}>Copy caption</GlassButton><GlassButton variant="ghost" onClick={() => void copyWithFeedback(`${item.id}-script`, String((pkg.sections?.short_script ?? pkg.sections?.short_reel_script ?? "")))}>Copy script</GlassButton><GlassButton variant="ghost" onClick={() => void copyWithFeedback(`${item.id}-cta`, normalizeCopyValue((pkg.sections?.CTA_options ?? pkg.sections?.cta_options ?? []) as string | string[]))}>Copy CTA options</GlassButton>{pkgStatus === "approved" ? <><GlassButton variant="secondary">Copy/export package</GlassButton><GlassButton variant="ghost">Generate image</GlassButton><GlassButton variant="ghost">Generate video</GlassButton><GlassButton variant="ghost">Go to publish/export</GlassButton></> : null}</div>{Object.entries(copiedState).some(([key, value]) => key.startsWith(item.id) && value) ? <p className="mt-2 text-emerald-300">Copied to clipboard.</p> : null}</div> : null}<div className="mt-2"><Link href={`/shopreel/campaigns/items/${item.id}?from=workspace`}><GlassButton variant="ghost" className="min-h-11 w-full sm:w-auto">Open output</GlassButton></Link></div></div>})}
          </div>
        </div>
