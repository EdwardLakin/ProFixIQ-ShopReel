...
export function resolveRouteFromPrompt(prompt: string, lastRoute?: string): RouteDecision {
  ...
  if (/\b(settings|workspace settings|billing)\b/.test(q)) return { intent: "settings", route: "/shopreel/settings", reason: "Settings intent.", usedRecovery: false, ignoredStaleContinuity: true, matched: true };
  ...
  if (/\b(advertis(e|ing)|facebook ads?|google ads?|launch [a-z0-9][\w\s-]*|week of posts?|content calendar|bookings?|leads?|new bakery|starting .* business|posts? for my .* business)\b/.test(q))
    return { intent: "create_campaign", route: "/shopreel/campaigns/new", reason: "Business/creative marketing prompt routed to campaign creation.", usedRecovery: false, ignoredStaleContinuity: true, matched: true };
