...
export default function RuntimeWorldWorkspaceCanvas({ entry, children, embodied }: { entry: RuntimeWorldEntry; children: ReactNode; embodied: RuntimeEmbodiedState }): RuntimeWorldWorkspacePlanes {
  ...
  return {
    ...
    midground: <section className={`mx-auto w-full overflow-hidden rounded-[1.65rem] border border-white/10 bg-[linear-gradient(180deg,rgba(8,16,30,.88),rgba(3,8,20,.92))] p-3 shadow-[0_36px_96px_rgba(2,6,23,.58)] ${isCampaignWorld ? "max-w-6xl" : ""}`}><RuntimeRoutePanelAdapter adapter={{ panelId: primary?.id ?? "primary", route: primary?.route ?? entry.href, title: primary?.title ?? "Manual workspace", embedMode: "embedded" }}>{children}</RuntimeRoutePanelAdapter></section>,
    ...
    peripheral: <div className="mx-auto grid w-full max-w-6xl gap-3 md:grid-cols-2" data-rhythm={embodied.operationalRhythm}>{secondary.map((panel, index) => <div key={panel.id} className={`rounded-[1.1rem] border border-white/10 bg-[linear-gradient(175deg,rgba(10,18,35,.70),rgba(5,10,22,.86))] p-2 shadow-[0_22px_72px_rgba(2,6,23,.42)] ${index === 0 ? "md:translate-y-2" : "md:-translate-y-1"}`}><RuntimeRoutePanelAdapter adapter={{ panelId: panel.id, route: panel.route, title: panel.title, embedMode: "embedded" }} /></div>)}</div>,
