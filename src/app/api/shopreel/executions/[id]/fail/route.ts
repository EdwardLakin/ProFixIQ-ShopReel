...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const execution = await transitionExecutionState({ shopId, executionId: id, userId, nextStatus: "failed", message: body.message ?? null });
    ...
    return toEndpointErrorResponse(error, "Failed to fail execution");
  }
}
