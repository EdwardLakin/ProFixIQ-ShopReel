...
export default async function ShopReelGenerationsPage() {
  ...
  return <GlassShell title="Projects" hidePageIntro><div className="space-y-4"><ShopReelPageHero title="Generations" subtitle="Operational cockpit for active renders, retries, and ready assets." actions={[{ label: "Create content", href: "/shopreel/create", primary: true }, { label: "Open render queue", href: "/shopreel/render-queue" }]} /><div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_290px]"><div className="space-y-4"><ShopReelSurface title="Queue health"><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{[{label:"Active renders",value:buckets["in-progress"].length},{label:"Queued jobs",value:generations.filter((g)=>normalizeStatus(g.status)==="queued").length},{label:"Failures",value:buckets.failed.length},{label:"Completed today",value:completedToday}].map((m)=><div key={m.label} className="rounded-xl border border-white/10 bg-black/25 p-3"><p className="text-xs text-white/60">{m.label}</p><p className="mt-1 text-2xl font-semibold text-white">{m.value}</p></div>)}</div></ShopReelSurface>
<ShopReelSectionHeader eyebrow="Generation lanes" title="Production visibility by lifecycle stage" subtitle="Track persisted generation states and route actions by operational need." />
