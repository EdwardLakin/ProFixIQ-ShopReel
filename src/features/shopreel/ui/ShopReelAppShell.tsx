    ...
    <div className={`min-h-screen bg-[#02040c] ${typography} ${embodiment.escalationPacing === "intense" ? "tracking-[0.01em]" : embodiment.calmExpansion === "expanded" ? "tracking-normal" : ""}`}>
      ...
      <div className={`relative flex min-h-screen overflow-x-clip ${railEmphasis} ${spatial.lanePosture === "compact" || embodiment.unstableCompression === "active" ? "gap-1" : spatial.lanePosture === "expanded" || embodiment.calmExpansion === "expanded" ? "gap-3" : "gap-2"} transition-[gap] duration-300`}>
        ...
        <section className={`relative min-w-0 flex-1 ${shellPadding} ${spatial.recoveryExpansion >= 70 || embodiment.recoveryBreathingRoom === "wide" ? "pt-3 pb-5" : spatial.instabilityCompression >= 70 || embodiment.unstableCompression === "active" ? "pt-1 pb-3" : "py-4"} ${transition?.safeLoading === "soft_continuity" ? "opacity-[0.97]" : transition?.mode === "continuity" ? "opacity-[0.97]" : embodiment.transitionPosture === "cooled" ? "opacity-[0.94]" : "opacity-100"} ${transition?.safeLoading === "soft_continuity" ? "transition-opacity duration-500" : ""} transition-[padding,opacity,transform] duration-300`}>
          <div className={`min-h-screen ${embodiment.transitionPosture === "forward" ? "[&_*]:transition-all" : ""} ${embodiment.surfaceWeight === "high" ? "space-y-2" : embodiment.calmExpansion === "expanded" ? "space-y-5" : "space-y-3"}`}><div className="flex flex-wrap items-center justify-between gap-2"><div className="min-w-0 flex-1 space-y-1"><GlobalEnvironmentAmbientLine />{transition ? <div className="text-[11px] text-cyan-100/70">{transition.routeCarryover} · {transition.descriptor.workflow_type}/{transition.descriptor.current_stage} → {transition.descriptor.next_stage} · Next: {transition.nextActionLabel}</div> : null}</div><div className="shrink-0"><GlobalCommandLauncher /></div></div>
            {children}</div>
        </section>
      </div>
    </div>
