...
{([ ["In progress", buckets["in-progress"]],["Ready to publish", buckets.completed],["Needs review", buckets.failed],["Drafts", buckets.drafts] ] as const).map(([label,items])=> <ShopReelSurface key={label} title={label} description={`${items.length} item${items.length===1?"":"s"}`}>{items.length===0?<ShopReelEmptyState title={`No ${label.toLowerCase()} right now`} description="Nothing waiting. Move the next draft forward."/>:<div className="grid gap-2">{items.map((g)=><GenerationCard key={g.id} generation={g} />)}</div>}</ShopReelSurface>)}
