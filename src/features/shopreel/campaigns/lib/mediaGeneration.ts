...
export function mergeLiveJobIntoMediaMetadata(metadata: unknown, liveJob: LiveMediaJobSnapshot | null | undefined): Json {
  if (!liveJob) return withMediaMetadata(metadata, {});
  const normalizedStatus = normalizeMediaStatus(liveJob.status);
  const patch: Record<string, unknown> = { image_status: normalizedStatus };
  if (typeof liveJob.preview_url === "string" && liveJob.preview_url.trim()) patch.image_preview_url = liveJob.preview_url;
  if (typeof liveJob.output_asset_id === "string" && liveJob.output_asset_id.trim()) patch.image_output_asset_id = liveJob.output_asset_id;
  if (normalizedStatus === "completed") {
    patch.image_completed_at = typeof liveJob.completed_at === "string" && liveJob.completed_at.trim()
      ? liveJob.completed_at
      : new Date().toISOString();
  }
  return withMediaMetadata(metadata, patch);
}
