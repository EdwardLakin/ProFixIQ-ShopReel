...
export function LandingAuthShell({
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#02040d] text-white">
      ...
      <div className="relative mx-auto grid min-h-screen w-full max-w-6xl items-center gap-8 px-6 py-10 lg:grid-cols-[1fr_440px]">
...
export function LandingInput(props: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  ...
  return (
    <label className="block">
      ...
      <input
        {...rest}
        className={`w-full rounded-2xl border border-white/10 bg-[#070b18]/78 px-4 py-3 text-white outline-none transition placeholder:text-white/32 hover:border-white/15 focus:border-cyan-300/35 focus:ring-2 focus:ring-cyan-300/20 ${className ?? ""}`}
      />
    </label>
  );
}
