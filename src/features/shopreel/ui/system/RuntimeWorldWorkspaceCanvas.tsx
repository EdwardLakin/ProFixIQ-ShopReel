...
export default function RuntimeWorldWorkspaceCanvas({ entry, children, embodied }: { entry: RuntimeWorldEntry; children: ReactNode; embodied: RuntimeEmbodiedState }): RuntimeWorldWorkspacePlanes {
  ...
  return {
    ...
    peripheral: <div className="grid gap-3 md:max-w-[28rem]" data-rhythm={embodied.operationalRhythm}>{secondary.map((panel) => <div key={panel.id} className="rounded-xl border border-white/12 bg-slate-950/62 p-2"><RuntimeRoutePanelAdapter adapter={{ panelId: panel.id, route: panel.route, title: panel.title, embedMode: "embedded" }} /></div>)}</div>,
